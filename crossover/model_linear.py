...
class ModelwiseLinearMerge(BaseModelMerger):
    ...
    def _generate_merge_params(self) -> np.ndarray:
        return np.array([0.0, self.std])

    def _merge(
        self, task_vectors: List[Dict], merge_params: np.ndarray
    ) -> Dict:
        num_task_vectors = len(task_vectors)

        assert (
            merge_params.size == 2
        ), f"Merge params must be of size 2 (1 mean, 1 std), got {merge_params.size}"

        weights = torch.normal(
            1.0 + float(merge_params[0]),
            abs(float(merge_params[1])),
